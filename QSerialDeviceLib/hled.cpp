...
struct HLed::Private {
    public:
    Private() : darkerFactor(300), color(Qt::green), isOn(true) { }
...
HLed::HLed(QWidget *parent) : QWidget(parent), m_d(new Private) {
    counter = 0;
    connect(&blinkTimer, SIGNAL(timeout()), this, SLOT(blinkOnce()));
}
...
void HLed::blink(qint64 count) {
    m_d->oldColor = m_d->color;
...
void HLed::paintEvent(QPaintEvent * /*event*/) {
    ...
    QColor color = m_d->isOn ? m_d->color : m_d->color.darker(m_d->darkerFactor);
